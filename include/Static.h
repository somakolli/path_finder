...
public:
    ...
    static inline void
    merge(ItA aBegin, ItA aEnd, ItB bBegin, ItB bEnd, Distance distanceToLabel, Container &result) {
        auto i = aBegin;
        auto j = bBegin;
        while (i < aEnd && j < bEnd) {
            if (i->id < j->id) {
    ...
    template<typename T>
    static inline std::vector<T> getFromFile(pathFinder::BinaryFileDescription fileDescription) {
    ...
    static inline auto getFromFileMMap(pathFinder::BinaryFileDescription fileDescription) {
        return pathFinder::MmapVector<T>(fileDescription.path.c_str(), fileDescription.size);
    }
};
}
