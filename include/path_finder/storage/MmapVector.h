...
MmapVector<T, Allocator>::MmapVector(const std::vector<T> &ramVector, const char *filename) {
  ...
  _data = (T *)mmap64(nullptr, sizeof(T) * ramVector.size(), PROT_READ, MAP_SHARED, fileno(file), 0);
...
MmapVector<T, Allocator>::MmapVector(const char *filename, size_t size) {
  ...
  auto mmapPointer = mmap64(nullptr, sizeof(T) * size, PROT_READ, MAP_SHARED, fileno(file), 0);
