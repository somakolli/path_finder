...
template <template <class, class> class Vector, class OffsetVector>
template <typename Id>
void HubLabelStore<Vector, OffsetVector>::store(
    const std::vector<CostNode> &label, Id id, EdgeDirection direction) {
  if (direction == EdgeDirection::FORWARD) {
    forwardOffset[id] =
        OffsetElement{forwardLabels.size(), (uint32_t)label.size()};
    for (auto element : label) {
      forwardLabels.push_back(element);
    }
  } else {
    backwardOffset[id] =
        OffsetElement{backwardLabels.size(), (uint32_t)label.size()};
    for (auto element : label) {
...
template <template <class, class> class Vector, class OffsetVector>
template <typename Id>
pathFinder::MyIterator<CostNode *>
HubLabelStore<Vector, OffsetVector>::retrieve(Id id, EdgeDirection direction) {
  if (direction == EdgeDirection::FORWARD) {
    auto offsetElement = forwardOffset[id];
    return MyIterator<CostNode *>{
        &forwardLabels[offsetElement.position],
        &forwardLabels[offsetElement.position + offsetElement.size]};
  } else {
    auto offsetElement = backwardOffset[id];
    return MyIterator<CostNode *>{
        &backwardLabels[offsetElement.position],
        &backwardLabels[offsetElement.position + offsetElement.size]};
  }
}
...
size_t HubLabelStore<Vector, OffsetVector>::getSpaceConsumption() {
  return forwardLabels.size() * sizeof(CostNode) +
         backwardLabels.size() * sizeof(CostNode);
}
} // namespace pathFinder
