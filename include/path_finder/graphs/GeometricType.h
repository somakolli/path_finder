...
struct LatLng {
  ...
  bool operator==(LatLng other) const { return other.lat == lat && other.lng == lng; }
  ...
  [[nodiscard]] double length() const { return std::sqrt(std::pow(lat, 2) + std::pow(lng, 2)); }
};
...
struct BoundingBox {
  ...
  [[nodiscard]] LatLng southWest() const { return LatLng(south, west); };
  [[nodiscard]] LatLng northEast() const { return LatLng(north, east); }
  [[nodiscard]] LatLng southEast() const { return LatLng(south, east); }
  [[nodiscard]] LatLng northWest() const { return LatLng(north, west); }
